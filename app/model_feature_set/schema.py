...
EXPECTED_MODEL_FEATURE_COUNT_V1 = 613
...
def _build_feature_names() -> tuple[str, ...]:
    ...
    for side in TEAM_SIDES:
        names.extend(
            f"{side}.starter_certainty.{value}"
            for value in _STARTER_CERTAINTY_VALUES
        )
        names.extend(
            f"{side}.lineup_availability.{value}"
            for value in _LINEUP_AVAILABILITY_VALUES
        )
        names.extend(f"{side}.coverage.{field}" for field in _COVERAGE_FIELDS)

        for window in FEATURE_WINDOWS:
            names.extend(
                f"{side}.starter.{window}.pitching.{field}"
                for field in _STARTER_PITCHING_FIELDS
            )
            names.extend(
                f"{side}.starter.{window}.pitch_traits.{field}"
                for field in _STARTER_TRAIT_FIELDS
            )
            names.extend(
                f"{side}.starter.{window}.pitch_physics.{field}"
                for field in _STARTER_PHYSICS_FIELDS
            )
            names.append(f"{side}.lineup.{window}.available_player_count")
            names.extend(
                f"{side}.lineup.{window}.hitting.{field}"
                for field in _LINEUP_HITTING_FIELDS
            )
            names.append(f"{side}.lineup.{window}.batted_ball_player_count")
            names.extend(
                f"{side}.lineup.{window}.batted_ball.{field}"
                for field in _LINEUP_BATTED_BALL_FIELDS
            )
            names.append(f"{side}.lineup.{window}.swing_player_count")
            names.extend(
                f"{side}.lineup.{window}.swing.{field}"
                for field in _LINEUP_SWING_FIELDS
            )
            names.append(f"{side}.bullpen.{window}.available_player_count")
            names.extend(
                f"{side}.bullpen.{window}.pitching.{field}"
                for field in _BULLPEN_PITCHING_FIELDS
            )
        ...
        names.extend(
            f"{side}.starter.previous_start.{field}"
            for field in _STARTER_RECENT_START_FIELDS
        )
        names.append(f"{side}.starter.previous_three_starts.start_count")
        for field in _STARTER_RECENT_START_FIELDS:
            names.append(f"{side}.starter.previous_three_starts.{field}_sum")
            names.append(f"{side}.starter.previous_three_starts.{field}_mean")

        names.extend(
            (
                f"{side}.bullpen.workload.available_player_count",
                f"{side}.bullpen.workload.days_since_previous_appearance_min",
                f"{side}.bullpen.workload.days_since_previous_appearance_mean",
                f"{side}.bullpen.workload.days_since_previous_start_min",
                f"{side}.bullpen.workload.days_since_previous_start_mean",
            )
        )
        for days in (1, 3, 7):
            for field in (
                "appearance_count_sum",
                "start_count_sum",
                "relief_appearance_count_sum",
                "pitch_count_sum",
            ):
                names.append(
                    f"{side}.bullpen.workload.previous_{days}_days.{field}"
                )

    names.extend(
        f"weather.status.{value}" for value in _WEATHER_STATUS_VALUES
    )
    names.extend(
        f"weather.relevance.{value}" for value in _WEATHER_RELEVANCE_VALUES
    )
...
FEATURE_INDEX_V1 = {
    name: index for index, name in enumerate(MODEL_FEATURE_NAMES_V1)
}
