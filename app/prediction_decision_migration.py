...
PREDICTION_DECISION_SCHEMA_V13_TABLE_STATEMENTS = (
    ...
    CREATE TABLE rankings_attempt_evidence (
        ...
        CHECK (json_extract(policy_json,'$.policy_version')='DSE_MLB_ML_LEXICOGRAPHIC_RANKING_V1'),
        CHECK (policy_checksum='536edd25604cdc0340e6c74bd3ff6bc67f24c75a6298cee63465a47da07ce0eb'),
        CHECK ((outcome='assembled' AND snapshot_checksum IS NOT NULL) OR (outcome<>'assembled' AND snapshot_checksum IS NULL)),
        CHECK (julianday(completed_at)>=julianday(created_at))
    )
    """,
...
PREDICTION_DECISION_SCHEMA_V13_VALIDATION_TRIGGER_STATEMENTS = (
    ...
    *tuple(
    ...
    WHEN OLD.sealed_at IS NULL AND NEW.sealed_at IS NOT NULL BEGIN
      ...
      SELECT CASE WHEN EXISTS (SELECT 1 FROM recommendation_gate_results r WHERE r.snapshot_id=NEW.snapshot_id AND r.passed=0 AND r.gate_code IN ('prediction_valid','prediction_market_independent','prediction_identity_valid','feature_lineage_valid')) THEN RAISE(ABORT,'Gate structural failure cannot be sealed') END;
