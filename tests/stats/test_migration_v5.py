...
def test_fresh_schema_installs_current_with_deterministic_fingerprint(
    ...
    assert first_result.version == second_result.version == 11
    assert first_result.schema_fingerprint == FORMAL_SCHEMA_V11_FINGERPRINT
    assert second_result.schema_fingerprint == FORMAL_SCHEMA_V11_FINGERPRINT
...
def test_ensure_schema_is_idempotent_at_current(tmp_path: Path) -> None:
    ...
    assert first.version == second.version == 11
    assert second.migrated is False
    assert second.schema_fingerprint == FORMAL_SCHEMA_V11_FINGERPRINT
