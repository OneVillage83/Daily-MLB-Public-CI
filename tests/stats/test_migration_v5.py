...
def test_fresh_schema_installs_current_with_deterministic_fingerprint(
    ...
    assert first_result.version == second_result.version == 10
    assert first_result.schema_fingerprint == FORMAL_SCHEMA_V10_FINGERPRINT
    assert second_result.schema_fingerprint == FORMAL_SCHEMA_V10_FINGERPRINT
...
def test_exact_v4_database_upgrades_transactionally_and_preserves_rows(
    ...
    assert result.schema_fingerprint == FORMAL_SCHEMA_V10_FINGERPRINT
    connection = sqlite3.connect(path)
    try:
...
def test_ensure_schema_is_idempotent_at_current(tmp_path: Path) -> None:
    ...
    assert first.version == second.version == 10
    assert second.migrated is False
    assert second.schema_fingerprint == FORMAL_SCHEMA_V10_FINGERPRINT
